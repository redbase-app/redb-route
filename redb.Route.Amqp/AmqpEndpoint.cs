...
public sealed class AmqpEndpoint : EndpointBase<AmqpEndpointOptions>
{
    ...
    {
        await EnsureConnectionAsync(ct).ConfigureAwait(false);
        return BuildReceiver(_session!, name);
    }

    /// <summary>
    /// Creates a receiver link on a FRESH dedicated <see cref="Session"/> (its own session on the
    /// pooled connection), returned alongside that session so the caller owns and closes both.
    /// <para>
    /// This is how <see cref="AmqpConsumer"/> runs <c>ConcurrentConsumers(N)</c> as N real competing
    /// consumers: AMQPNetLite <see cref="Session"/>/<see cref="Link"/> objects are NOT thread-safe, so
    /// concurrent receive/accept must not share one session. Each worker gets its own session +
    /// receiver and drives them from a single thread.
    /// </para>
    /// </summary>
    internal async Task<(Session Session, ReceiverLink Receiver)> CreateDedicatedReceiverAsync(
        string? name = null, CancellationToken ct = default)
    {
        ...
        try
        {
            var receiver = BuildReceiver(session, name);
            return (session, receiver);
        }
        catch
        {
            try { await session.CloseAsync().ConfigureAwait(false); } catch { /* best-effort */ }
            throw;
        }
    }

    /// <summary>Builds a configured <see cref="ReceiverLink"/> (source, filters, credit) on the given session.</summary>
    private ReceiverLink BuildReceiver(Session session, string? name)
    {
