...
- **22 external transports + 5 built-in components** (Direct, SEDA, Timer, Mock, Log).
...
- **MIT licensed** — no commercial restrictions, no per-endpoint pricing.
...
- **22 transports out of the box.** Kafka, RabbitMQ, Redis, SQL, HTTP, gRPC, SFTP, MQTT, S3, IBM MQ, AMQP 1.0, Azure Service Bus, Elasticsearch, Firebase, LDAP, Mail, TCP, WebSocket, SignalR, FTP, Quartz, File. MassTransit ships 5, Wolverine 4, NServiceBus 7. The competitors expect you to use only message brokers; redb.Route treats files, mailboxes, FTP servers and SQL polling as first-class transports.
...
- **MIT licensed, no per-endpoint pricing.** NServiceBus is commercial after 2 endpoints. redb.Route is unrestricted for any use, any scale.
...
| Durable saga state machine with DB persistence | MassTransit / NServiceBus / Wolverine |
| Transactional outbox pattern | MassTransit / NServiceBus / Wolverine |
| Mediator-style in-process handlers | Wolverine |
| Pure pub/sub with consumer classes | MassTransit |
...
| Focus | EIP routing | Pub/sub + Saga | Reliable bus | Mediator + bus | **EIP routing** |
| Transports | 300+ | 5 | 7 | 4 | **22 + 5 built-in** |
...
| Components | 300+ | 22 transports + 5 built-in |
...
|--------|-------------|------------|
| Focus | Reliable message bus + Saga | EIP routing |
...
Use MassTransit for **state-machine sagas with DB persistence** and a built-in transactional outbox table per consumer. Use redb.Route for EIP routing, protocol bridging, file processing, and multi-transport pipelines. The reliability primitives (publisher confirms, EOS, persistent dedup, polling outbox) are present — they are just composed from DSL building blocks rather than declared as a single `UseOutbox()` switch.

### vs NServiceBus

| Aspect | NServiceBus | redb.Route |
|--------|-------------|------------|
| Focus | Reliable enterprise service bus | EIP routing |
...
| License | **Commercial** (free up to 2 endpoints/app) | MIT |

NServiceBus is mature and focused on guaranteed delivery and durable saga. redb.Route is MIT-licensed and focused on routing logic, protocol bridging, and EIP patterns.

### vs Wolverine

| Aspect | Wolverine | redb.Route |
|--------|-----------|------------|
| Focus | Mediator + message bus + Saga | EIP routing |
...
Wolverine is excellent at handler-style messaging with Marten-backed sagas. redb.Route is focused on integration routing and protocol diversity.
...
flowchart TB
    ...
    subgraph Transports["redb.Route.* — 22 transports"]
...
| Amqp | ✅ | ✅ | ✅ | — | — | AMQP 1.0; RPC via `ReplyTo`/`CorrelationId`, `ExchangePattern.InOut` auto-detected |
...
| `redb.Route.Core` | [![NuGet](https://img.shields.io/nuget/v/redb.Route.Core?label=)](https://www.nuget.org/packages/redb.Route.Core) | Bridge to redb.Core EAV storage |
...
27 packages total: core engine + 22 transports + 5 support libraries.
