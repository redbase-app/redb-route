...
/// <summary>
/// Where a hub looks for the shared Kestrel host: explicitly assigned, then the context's DI
...
/// </summary>
public class SignalRSharedHostFromDiTests
{
    ...
    public void The_manager_in_the_contexts_container_is_used_when_none_was_assigned()
    {
        ...
        component.EffectiveServerManager.Should().BeSameAs(shared);
    }
    ...
    public void An_assigned_manager_wins_over_the_container()
    {
        ...
        var component = new SignalRComponent { ServerManager = assigned };

        using var context = new RouteContext();
        context.SetServiceProvider(ProviderWith(new SharedHttpServerManager()));
        context.AddComponent(component);

        component.EffectiveServerManager.Should().BeSameAs(assigned);
    }
    ...
    public void Without_either_the_component_still_owns_one()
    {
        ...
        // new SignalRComponent() stays usable on its own — tests, embedded hosts.
        component.EffectiveServerManager.Should().NotBeNull();
    }
}
