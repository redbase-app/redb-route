...
public sealed class RedbKnowledgeStore : IKnowledgeStore
{
    ...
    public async Task UpsertManyAsync(IEnumerable<KnowledgeChunk> chunks, IExchange? exchange = null, CancellationToken ct = default)
    {
        ...
        foreach (var chunk in input)
        {
            ...
            {
                // Hash-pre-check: SaveAsync's change-tracking re-reads the row
                // to diff against the stored copy. Comparing pre/post hash in
                // memory skips unchanged rows entirely — zero round-trips for
                // idempotent re-upserts.
                var hashBefore = existing.ComputeHash();
                Populate(existing, chunk, out var same);
                if (same.ComputeHash() == hashBefore) continue;
                rowsToSave.Add(same);
            }
            else
            {
